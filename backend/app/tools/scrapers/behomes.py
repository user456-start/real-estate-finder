"""
Behomes scraper — Playwright + network interception.

Behomes (behomes.tech) is a UAE property aggregator / MLS platform.

To verify / update the API endpoint:
    Open behomes.tech in Chrome DevTools → Network → XHR/Fetch
...
_BASE = "https://behomes.tech"

_API_PATTERNS = ("/api/properties", "/api/search", "/api/listings", "/api/v", "/graphql")
...
class BehomesScraper(BaseScraper):
    platform_name = "behomes"
    page_size = 20
    max_pages = 5
    ...
    async def fetch_page(self, preferences: dict[str, Any], page: int) -> list[RawListing]:
        ...
        for area in areas:
            ...
            url = f"{_BASE}/properties/{purpose}/apartments/{slug}?page={page}"
    ...
    async def _fetch_area_page(self, url: str, preferences: dict[str, Any]) -> list[RawListing]:
        ...
        async def handle_response(response):
            if any(p in response.url for p in _API_PATTERNS) and response.status == 200:
                try:
                    ...
                    # Handle both array root and wrapped responses
        ...
        try:
            ...
            logger.warning("[behomes] page load error: %s", exc)
        finally:
            await page.close()
    ...
    async def _extract_next_data(self, url: str) -> list[dict]:
        ...
        try:
            ...
            logger.warning("[behomes] __NEXT_DATA__ extraction failed: %s", exc)
        finally:
            await page.close()
        return props_list
